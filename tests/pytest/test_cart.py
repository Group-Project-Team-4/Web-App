...
def test_add_to_cart(client, app, auth):
    ...
    auth.login()
    response = client.post("/cart", data={"product_id": 3, "quantity": 1})
    print(response)
    ...
    with app.app_context():
        assert (
            get_db()
            .execute(
                "SELECT * FROM cart_item WHERE user_id = '1' AND product_id = '3'",
            )
            .fetchone()
            is not None
        )
    ...
    auth.login()
    response = client.post("/cart", data={"product_id": 3, "quantity": 100})
    assert response.status_code == 302
    with app.app_context():
        assert (
            get_db()
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ({"product_id": 1}, b"Quantity is required."),
